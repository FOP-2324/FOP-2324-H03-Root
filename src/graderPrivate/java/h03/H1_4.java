...
import static h03.robots.Robots_Student.MultiFamilyRobot_Student.MULTI_FAMILY_ROBOT_MOVE_WITHOUT_PARAMETER_L;
...
public class H1_4 {
    ...
    public void testMethodReturnType() {
        assertCorrectReturnType(MULTI_FAMILY_ROBOT_MOVE_WITHOUT_PARAMETER_L.get(), sameType(VOID_LINK));
    }

    @Test
    public void testMethodModifiers() {
        assertCorrectModifiers(MULTI_FAMILY_ROBOT_MOVE_WITHOUT_PARAMETER_L.get(), PUBLIC, NON_STATIC);
    }
    ...
    public void testSuperCall() {
        ...
        if (numberOfCalls != 1) {
            fail(
                contextBuilder().subject(MULTI_FAMILY_ROBOT_MOVE_WITHOUT_PARAMETER_L.get()).build(),
    ...
    public void testExchangeCall() {
        ...
        verifyX(
            ...
            () -> robot.move(context(parameters)),
            context(parameters),
            r -> "%s method was not called one time".formatted(tt("exchange"))
        );
    }
